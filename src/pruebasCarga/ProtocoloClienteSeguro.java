...
public class ProtocoloClienteSeguro {
	...
	InvalidKeyException, IllegalBlockSizeException, SignatureException{
		...
		System.out.println("Escribiendo los Algoritmos a usar...");
		...
		String llaveSimetricaCifrada = DatatypeConverter.printBase64Binary(cifrar(llavePublica, ALG_A, llaveSimetrica.getEncoded()));
			
		
		pOut.println(llaveSimetricaCifrada);
		System.out.println("Escriba reto:");
		String reto = randString(12);
		pOut.println(reto);
		if((fromServer = pIn.readLine()) != null){
			if(fromServer.equals("ERROR")) System.exit(-1);
			System.out.println("Reto Cifrado: " + fromServer); //Reto Cifrado
		}
		
		String retoCifrado =  fromServer;
		String retoCifradoLocalmente = DatatypeConverter.printBase64Binary((cifrar(llaveSimetrica, ALG_S[randomALGS], DatatypeConverter.parseBase64Binary(reto))));

		System.out.println("Reto Cifrado localmente: " + retoCifradoLocalmente);
		
		if(retoCifrado.equals(retoCifradoLocalmente)) {System.out.println("Autenticación Exitosa");pOut.println("OK");}
		else pOut.println("ERROR");
		
		//ETAPA 3//
		System.out.println("Escriba cc:");
		String cc = randString(12);
		String ccCifrado = DatatypeConverter.printBase64Binary((cifrar(llaveSimetrica, ALG_S[randomALGS], cc.getBytes())));
		
		System.out.println("Escriba clave:");
		String clave = randString(16);
		String claveCifrada = DatatypeConverter.printBase64Binary((cifrar(llaveSimetrica, ALG_S[randomALGS], clave.getBytes())));
		
		pOut.println(ccCifrado); 
		pOut.println(claveCifrada);
		
		if((fromServer = pIn.readLine()) != null){
			if(fromServer.equals("ERROR")) System.exit(-1);
			System.out.println("Recibe Valor Cifrado"); 
		}
		String valorCifradoKS = fromServer; //CKS(<valor>)
		
		if((fromServer = pIn.readLine()) != null){
			if(fromServer.equals("ERROR")) System.exit(-1);
			System.out.println("Recibe Codigo de autenticacion"); 
		}
		String valorCifradoHMAC = fromServer; //CKW-(HMAC_KS(<valor>))
		
		String ValorClaro = DatatypeConverter.printBase64Binary(descifrar(llaveSimetrica, ALG_S[randomALGS], DatatypeConverter.parseBase64Binary(valorCifradoKS)));
		System.out.println("VALOR descifrado: " + ValorClaro);

		String ValorHMAC = DatatypeConverter.printBase64Binary(descifrar(llavePublica, ALG_A, DatatypeConverter.parseBase64Binary(valorCifradoHMAC))); //
		System.out.println("VALOR HMAC: " + ValorHMAC);
		
		//TODO HACER HMAC y AUTENTICAR
		...
		if(ValorHMAC.equals(ValorHMACLocal)) {System.out.println("Autenticacion Exitosa");pOut.println("OK");}
		else pOut.println("ERROR");
